...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            ...
            const biography = '📅 ' + date_bio + '\n⌚ ' + timezone_bio + '\nby *~♥️🕊️______ 𝐏𝐈𝐊𝐀𝐂𝐇𝐔 _____🕊️♥️~*'
    ...
    var ttq7 = ''
    var ttl8 = ''
    await axios.get('https://gist.github.com/ameer-kallumthodi/b033bb0699e32d214e6a5c3a3648405e/raw/').then(async (insult) => {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    WhatsAsenaCN.on('credentials-updated', async () => {
        ...
        console.log(
            chalk.green.bold('Installed plugins...')
        );
        if (os.userInfo().homedir !== clh.pay) return;
        asynchronous_ch()
        await new Promise(r => setTimeout(r, 200));
        let afwhasena = config.WORKTYPE == 'public' ? ' Public' : ' Private'
        console.log(chalk.bgGreen('Pikachu Running! 😎'));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = '*¡*~♥️🕊️______ 𝐏𝐈𝐊𝐀𝐂𝐇𝐔 _____🕊️♥️~* bot funciona como Chatbot!* 🐺\n\n_El propósito de este mod es convertir el bot en una herramienta de chat de IA completamente funcional._\n_Puede utilizar el comando_ */fulleva off* _para volver al modo normal._\n\n*Gracias por usar *~♥️🕊️______ 𝐏𝐈𝐊𝐀𝐂𝐇𝐔 _____🕊️♥️~* Bot ❤️‍🔥*\n    *- *~♥️🕊️______ 𝐏𝐈𝐊𝐀𝐂𝐇𝐔 _____🕊️♥️~**'
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                if (error.message.includes('URL')) {
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*⚕️ ANÁLISIS DE ERRORES [*~♥️🕊️______ 𝐏𝐈𝐊𝐀𝐂𝐇𝐔 _____🕊️♥️~*] ⚕️*' + 
                                        '\n========== ```¡Error resuelto!``` ==========' +
				                    	'\n\n*Max Error oyvakkan Subscribe cheyyu* https://www.youtube.com/channel/UCpGa88rhUFYj-6-LBqBbWKw' +
